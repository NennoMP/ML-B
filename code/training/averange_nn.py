...
def averange_prediction_nn(estimator, params, N_epochs, x_dev, x_test, y_dev, y_test, k, x_blind_test=None, optimizer=None):
    ...
    if(x_blind_test is not None): # CUP
        ...
        acc_dev = []
        acc_test = [] 
    if(optimizer is not None): # Ensemble
        prediction_dev = []
        prediction_test = []
    ...
    for i in range(k):
        ...
        if(optimizer is not None): # Ensemble
            ...
            if(x_blind_test is None): # MONK Ensemble
                pred_dev = (np.rint(pred_dev)).astype(int)
    ...
    if(x_blind_test is not None): # CUP
        ...
        if(optimizer is not None): # CUP Ensemble
            ...
            return prediction_blind_test, mse_mean_dev, mse_mean_test, mse_std_dev, mse_std_test, mee_mean_dev, mee_mean_test, mee_std_dev, mee_std_test
        ...
        if(optimizer is not None): # MONK Ensemble 
            prediction_dev = np.mean(np.array(prediction_dev), axis=0)
            prediction_test = np.mean(np.array(prediction_test), axis=0)    
            return prediction_dev, prediction_test, mse_mean_dev, mse_mean_test, mse_std_dev, mse_std_test, acc_mean_dev, acc_mean_test, acc_std_dev, acc_std_test, bce_mean_dev, bce_mean_test
        else: # MONK
            return mse_mean_dev, mse_mean_test, mse_std_dev, mse_std_test, acc_mean_dev, acc_mean_test, acc_std_dev, acc_std_test, bce_mean_dev, bce_mean_test
