...
def averange_prediction_nn(estimator, params, N_epochs, x_dev, x_test, y_dev, y_test, k, x_blind_test=None, optimizer=None):
    ...
    for i in range(k):
        ...
        if(x_blind_test is not None): #CUP
            ...
            solver = Solver(model, x_dev, y_dev, target='loss')
            ...
            bce_monk_dev, acc_monk_dev, mse_e_dev = model.evaluate(x_dev, y_dev)
            bce_monk_test, acc_monk_test, mse_e_test = model.evaluate(x_test, y_test)
            bce_dev.append(bce_monk_dev)
            bce_test.append(bce_monk_test)
    ...
    if(x_blind_test is not None):
        ...
        bce_mean_dev = np.mean(np.array(bce_dev))
        bce_mean_test = np.mean(np.array(bce_test))
        bce_std_dev = np.std(np.array(bce_dev))
        bce_std_test = np.std(np.array(bce_test))
              
        return prediction_dev, prediction_test, mse_mean_dev, mse_mean_test, mse_std_dev, mse_std_test, acc_mean_dev, acc_mean_test, acc_std_dev, acc_std_test, bce_mean_dev, bce_mean_test, bce_std_dev, bce_std_test
